(() => {
    ...
    if(shouldSpoof) {
        if (typeof EnterKioskMode !== 'undefined') EnterKioskMode();
    }

    if(shouldSpoof) setInterval(() => {
        if((screen.height - innerHeight > 5) || (screen.width - innerWidth > 5))
            if (typeof WhoopsieDaises !== 'undefined') WhoopsieDaises();
    ...
    unsafeWindow.chrome.runtime.sendMessage = unsafeWindow.chrome.runtime.sendMessage || function(extId, payload, callback){
        if (typeof chrome !== 'undefined' && chrome.runtime && chrome.runtime.lastError) {
            chrome.runtime.lastError = { message: "Not implemented" };
        }
    ...
    function PageIsErrored() {
        ...
        if (ChildNodes[3] && ChildNodes[3].getAttribute("aria-live") === "assertive" && 
            ChildNodes[4] && ChildNodes[4].getAttribute("aria-live") === "assertive") {
    ...
    async function IsOnChromebook() {
        return new Promise((resolve, _reject) => {
            oldSendMessage(kAssessmentAssistantExtensionId, {command: "isLocked"}, function(response) {
